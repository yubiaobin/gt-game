...
function onDeviceReady() {
    ...
    // 定义基础的模型类
    var ClassicList2Model = Backbone.Model.extend({
        defaults : {
            title     :    '',
            content   :    '',
            goodCount :    '',
    ...
    // 创建集合模型类
    var ClassicLists2 = Backbone.Collection.extend({
        model       :  ClassicList2Model, 
        url         :  function(){
            return "http://115.29.201.207:8008/playoutjson?mac=" + this.uuid;
            // return "data35.json"
        },
    ...
    // 实例化
    var classicData2 = new ClassicLists2;


    // 创建单个item的view对象
    var ClassicListItemView2 = Backbone.View.extend({
        tagName     :  "li",
        template    :  _.template($('#GameList22ItemView_template').html()),
        initialize  : function(){
    ...
    // 创建首页的View对象
    var ClassicListView2 = Backbone.View.extend({
        el             :   $("#gtgame_played_yes_data"),
        initialize     :   function() {
            this.listenTo(this, 'add', this.render);
            this.listenTo(this, 'reset', this.render);
            this.listenTo(this, 'all', this.render);
            classicData2.fetch({
                success  :  function(model,response){
                        classicData2.each(function (object) {
                            // alert("获取数据成功");
                            console.log(object.toJSON());
                            var view = new ClassicListItemView2({model: object});
                            this.$("#gtgame_played_yes_data").append(view.render().el);
                        });
                        console.log(classicData2.toJSON());
    ...
    // 实例化View对象
    var gtgame_classic2 = new ClassicListView2;
    ...
    var ClassicLists = Backbone.Collection.extend({
        ...
        url         :  function(){
            return "http://115.29.201.207:8008/tuijian"
            // return "data15.json"
        },
        parse       :  function(response){
            return response.data;
            ...
            classicData.fetch({
                success  :  function(model,response){
                        classicData.each(function (object) {
                            ...
                            this.$("#gtgame_played_no_data").append(view.render().el);
                        });
